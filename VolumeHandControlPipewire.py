...
def set_volume(percentage):
    """
    Set the system volume to a specific percentage using pactl.
    :param percentage: Volume percentage (0 to 100).
    """
    try:
        # Convert the percentage to a value pactl can use (65536 is 100% in pactl)
        volume = int(65536 * percentage / 100)
        # Use pactl to set the volume
        subprocess.run(['pactl', 'set-sink-volume', '@DEFAULT_SINK@', str(volume)])
